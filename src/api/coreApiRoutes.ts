...
export function createCoreApiRouter(authenticateToken: any): Router {
  ...
  router.get('/tenant', authenticateToken, (req: AuthenticatedCoreRequest, res) => {
    ...
    if (!tenant) return res.status(404).json({ success: false, error: 'Không tìm thấy Tenant' });
    return res.json({ success: true, tenant });
  });

  router.put('/tenant', authenticateToken, requireCorePermission('tenant.update'), enforceDataScopeMutation(), (req: AuthenticatedCoreRequest, res) => {
    const tenantId = req.user?.tenantId || req.user?.tenant || 'tenant-household-01';
  ...
  router.get('/branches', authenticateToken, (req: AuthenticatedCoreRequest, res) => {
    const tenantId = req.user?.tenantId || req.user?.tenant || 'tenant-household-01';
    return res.json({ success: true, branches: TenantService.getTenantBranches(tenantId) });
  });

  router.post('/branches', authenticateToken, requireCorePermission('branch.create'), enforceDataScopeMutation(), (req: AuthenticatedCoreRequest, res) => {
    const tenantId = req.user?.tenantId || req.user?.tenant || 'tenant-household-01';
  ...
  router.get('/warehouses', authenticateToken, (req: AuthenticatedCoreRequest, res) => {
    ...
    const warehouses = branchId ? TenantService.getBranchWarehouses(tenantId, branchId) : TenantService.getTenantWarehouses(tenantId);
    return res.json({ success: true, warehouses });
  });

  router.post('/warehouses', authenticateToken, requireCorePermission('warehouse.create'), enforceDataScopeMutation(), (req: AuthenticatedCoreRequest, res) => {
    const tenantId = req.user?.tenantId || req.user?.tenant || 'tenant-household-01';
  ...
  router.get('/users', authenticateToken, requireCorePermission('user.view'), (req: AuthenticatedCoreRequest, res) => {
    const tenantId = req.user?.tenantId || req.user?.tenant || 'tenant-household-01';
    return res.json({ success: true, users: UserService.getUsersByTenant(tenantId) });
  });

  router.post('/users', authenticateToken, requireCorePermission('user.create'), enforceDataScopeMutation(), (req: AuthenticatedCoreRequest, res) => {
    const tenantId = req.user?.tenantId || req.user?.tenant || 'tenant-household-01';
    const result = UserService.createUser(tenantId, req.body, req.securityContext!);
    if (!result.success) return res.status(400).json({ success: false, error: result.error });
  ...
  router.post('/fad-migrations/import', authenticateToken, requireCorePermission('migration.fad.import'), enforceDataScopeMutation(), (req: AuthenticatedCoreRequest, res) => {
    ...
    const result = FadMigrationService.importManifest(tenantId, userId, req.body as FadMigrationManifestInput);
    if (!result.success) return res.status(422).json({ success: false, migration: result.migration });
    AuditLogService.log(tenantId, userId, 'IMPORT', 'migration', 'FAD_MIGRATION', result.migration.migrationId, { sourceSystem: 'FAD', importedCount: result.migration.importedCount, skippedCount: result.migration.skippedCount });
    return res.json({ success: true, migration: result.migration });
  });

  router.get('/fad-migrations/history', authenticateToken, requireCorePermission('migration.fad.view'), (req: AuthenticatedCoreRequest, res) => {
    const tenantId = req.user?.tenantId || req.user?.tenant || 'tenant-household-01';
    const limit = req.query.limit ? parseInt(req.query.limit as string, 10) : 50;
    return res.json({ success: true, history: FadMigrationService.getHistory(tenantId, limit) });
  });

  router.get('/fad-migrations/records', authenticateToken, requireCorePermission('migration.fad.view'), (req: AuthenticatedCoreRequest, res) => {
    const tenantId = req.user?.tenantId || req.user?.tenant || 'tenant-household-01';
  ...
  router.post('/tests/run', (req, res) => {
    ...
    return res.json({ success: testReport.failed === 0, report: testReport });
  });
